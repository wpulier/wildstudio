...
export default function LegendaryCursor() {
    ...
    useEffect(() => {
        ...
        // Physics parameters - increased spring and reduced friction for more direct connection
        const spring = 0.25; // Increased from 0.08
        const friction = 0.85; // Increased from 0.75
        const trailStrength = 0.95; // Increased from 0.85
        ...
        const animate = () => {
            ...
            for (let i = 0; i < points.length; i++) {
                ...
                if (i === 0) {
                    // First point follows mouse more directly
                    point.x = mouseX - (mouseVelocityX * 0.5); // Slight offset based on velocity
                    point.y = mouseY - (mouseVelocityY * 0.5);
                    point.vx = mouseVelocityX;
                    point.vy = mouseVelocityY;
                } else {
            ...
            if (points.length > 2) {
                ...
                const baseWidth = 4; // Increased from 2
                const maxWidth = 12; // Increased from 8
                const dynamicWidth = baseWidth + (speed / 8);
                ...
                const intensity = Math.min(0.8 + speed / 100, 1);

                // Multiple color stops for sparkly effect
                gradient.addColorStop(0, `rgba(255, 255, 255, ${intensity})`);
                gradient.addColorStop(0.2, `rgba(70, 158, 255, ${intensity * 0.95})`);
                gradient.addColorStop(0.4, `rgba(128, 0, 255, ${intensity * 0.9})`);
                gradient.addColorStop(0.7, `rgba(255, 0, 128, ${intensity * 0.85})`);
                gradient.addColorStop(1, 'rgba(0, 0, 0, 0)');

                // Add glow effect
                ctx.shadowColor = 'rgba(255, 255, 255, 0.9)';
                ctx.shadowBlur = 8 + speed / 8;
                ...
                if (speed > 8) { // Reduced threshold from 10
                    const particleCount = Math.floor(speed / 4);
                    for (let i = 0; i < particleCount; i++) {
                        const pointIndex = Math.floor(Math.random() * (points.length - 2)) + 1;
                        const point = points[pointIndex];
                        const size = Math.random() * 4 + 2; // Increased from 3+1
